...
name: speckit-review
description: >-
  Review a GitHub PR against this repository's constitution: Spec Kit
  artifact consistency, the Implementation Heuristic Gate (Principle VI),
  coverage floor, local CI checks run on the PR's actual code, and
  ctxlite-internals compliance for fragile areas. Reports findings only —
  never posts to GitHub automatically. Takes a PR number or URL as input.
---
...
10. **ctxlite-internals compliance** — if the PR's changed files (from step 3's `files` list) include any of `packages/core/src/tool-precall.ts`, anything under `packages/core/src/install/`, or `packages/core/src/stats.ts`:
    - Re-read `.claude/skills/ctxlite-internals/SKILL.md` for the specific rules tied to whichever file(s) are touched.
