...
describe("extractSymbols", () => {
  ...
  it("SC-001: symbols output is ≥60% smaller than full file for large fixture", async () => {
    const { readFile } = await import("node:fs/promises")
    const { join, dirname } = await import("node:path")
    const { fileURLToPath } = await import("node:url")
    const { estimateTokens } = await import("./tokens.js")
    const fixtureDir = join(dirname(fileURLToPath(import.meta.url)), "../test-fixtures/mcp-efficiency")
    const full = await readFile(join(fixtureDir, "large-file.ts"), "utf8")
    const symbols = await extractSymbols(full, "large-file.ts")
    expect(symbols).not.toBeNull()
    const ratio = 1 - estimateTokens(symbols!) / estimateTokens(full)
    expect(ratio).toBeGreaterThanOrEqual(0.6)
    expect(symbols).toContain("export class AuthService")
    expect(symbols).toContain("export function validateEmail")
  })
})
