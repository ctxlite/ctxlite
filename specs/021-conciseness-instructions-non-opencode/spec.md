...
**Independent Test**: Run `ctxlite install --tool claude-code --scope project --yes` in a fresh project; confirm a ctxlite-owned rule file under `.claude/rules/` contains the conciseness instructions. Run `ctxlite install --tool cursor --scope project --yes`; confirm a ctxlite-owned Cursor rule file under `.cursor/rules/` contains the same instructions.

**Acceptance Scenarios**:

1. **Given** a project with no existing ctxlite Claude Code rule file, **When** `ctxlite install --tool claude-code` runs, **Then** a dedicated, ctxlite-owned file is created at `.claude/rules/ctxlite-conciseness.md` (project scope) or `~/.claude/rules/ctxlite-conciseness.md` (global scope) containing the conciseness instructions — Claude Code loads every file under `.claude/rules/` unconditionally at session start, the same way it already loads `CLAUDE.md`, so this requires no merge into the user's own `CLAUDE.md` at all (verified against Claude Code's current documentation: rules without `paths` frontmatter "are loaded at launch with the same priority as `.claude/CLAUDE.md`").
2. **Given** a project with an existing `CLAUDE.md` or other `.claude/rules/*.md` files, **When** `ctxlite install --tool claude-code` runs, **Then** none of that content is touched — ctxlite's rule file is a sibling file it fully owns, the same ownership model already used for `.claude/skills/ctxlite/SKILL.md`, not a block merged into shared content.
3. **Given** `ctxlite install --tool claude-code` has already run once, **When** it runs again, **Then** the file's content is unchanged (idempotent — writing the same content again is a no-op, same as the skill-file install already does).
4. **Given** `ctxlite install --tool cursor` runs, **Then** a ctxlite-owned Cursor rule file is created at `.cursor/rules/ctxlite-conciseness.mdc` (project scope) or `~/.cursor/rules/ctxlite-conciseness.mdc` (global scope) — `.mdc` frontmatter (`description`, `alwaysApply: true`) matching this project's own existing `.cursor/rules/*.mdc` files (e.g. `security.mdc`), so it loads unconditionally the same way.
5. **Given** `ctxlite uninstall` (or the install command's removal path) runs for either host, **Then** ctxlite's own rule file is deleted — no merge/un-merge logic needed since nothing else shares the file.

### Edge Cases

- What happens to a project that installed ctxlite before this feature shipped? → Re-running `ctxlite install` for that tool adds the new rule file — consistent with this project's own documented precedent ("`ctxlite install` only writes a *new* install target on a run *after* that target was added... expected, not a bug, but call it out in the CHANGELOG" — `ctxlite-internals` skill, `core/install/` section).
- What happens if a user has manually written conflicting instructions elsewhere (their own `CLAUDE.md`, a different rule file)? → Out of scope to detect or reconcile — ctxlite's rule file is a separate, additive file; resolving conflicting instructions across files is the user's own editorial decision, not something this feature arbitrates.
- What happens for `--scope global` versus `--scope project`? → Both supported, writing to `~/.claude/rules/`/`~/.cursor/rules/` (global) or `.claude/rules/`/`.cursor/rules/` (project) respectively, mirroring the existing scope handling already used for hooks/skills/MCP config.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST deliver the existing `CONCISENESS_INSTRUCTIONS` text (currently OpenCode-only) to Claude Code via a dedicated, ctxlite-owned file under `.claude/rules/` when `ctxlite install --tool claude-code` runs — loaded unconditionally by Claude Code, same as `CLAUDE.md`, with no merge into the user's own files required.
- **FR-002**: System MUST deliver the same instructions to Cursor via a dedicated, ctxlite-owned file under `.cursor/rules/` (`.mdc` with `alwaysApply: true`, matching this project's own existing rule files) when `ctxlite install --tool cursor` runs.
- **FR-003**: Both delivery mechanisms MUST follow this project's existing full-file-ownership install convention (create if absent, overwrite with identical content on re-run — idempotent, clean delete on uninstall) — the same pattern already used for `.claude/skills/ctxlite/SKILL.md`/`.cursor/skills/ctxlite/SKILL.md`, not a new pattern invented for this feature, and notably simpler than the merge-block pattern hooks use since nothing else shares the file.
...
- **SC-001**: A Claude Code or Cursor session, after a fresh `ctxlite install`, receives the same token-efficiency instructions (skip preamble/recap/sign-off, etc.) an OpenCode session already receives — verifiable by inspecting the installed rule file's content directly.
- **SC-002**: Running `ctxlite install` twice for the same tool produces byte-identical output the second time (true idempotency) — no duplicate content, no spurious file changes, across repeated installs.
...
- Claude Code's `.claude/rules/*.md` (confirmed via current documentation: files without `paths` frontmatter load unconditionally, same priority as `CLAUDE.md`) and this project's own already-working `.cursor/rules/*.mdc` convention (`alwaysApply: true`) are both real, already-functioning "always-loaded instructions" mechanisms — verified directly rather than assumed, exactly mirroring the file-ownership model `.claude/skills/ctxlite/SKILL.md` already uses (a dedicated file ctxlite fully owns, never merged into shared content).
- No new external dependency needed — this reuses the existing full-file install/write logic pattern already proven for skill installation, just targeting two new file paths with different (but simpler, since no merge needed) content.
